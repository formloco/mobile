...
const formRegisterSQL = async (data) => {
  ...
  if (form.rowCount === 0) {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS public.form ("id" int4 NOT NULL DEFAULT nextval('form_id_seq'::regclass),form_id uuid, tenant_id uuid, name varchar, form jsonb, pin varchar, date_last_access timestamp DEFAULT now(), date_created timestamp DEFAULT now(), date_archived timestamp, user_created jsonb, user_updated jsonb, user_archive int4, is_data bool, is_list bool, type varchar, is_published bool, is_manager, PRIMARY KEY ("id"))`)
...
const formPublishSQL = async (data) => {
  ...
  client.release()
  const forms = await client.query(`SELECT * FROM public.form WHERE date_archived is null AND type = 'dynamic'`)
  return forms.rows
}
