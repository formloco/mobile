...
import {
  SetBackground,
  SetIsDarkMode,
} from '../state/device/device-state.actions';
import {
  SetPage,
  SetChildPage,
  SetLookupListData,
...
import {
  SetNotificationOpen,
  SetNotificationSigned,
  SetNotificationAllOpen,
  SetNotificationAllSigned,
  SetNotificationMyCount,
  SetNotificationAdminCount,
...
import {
  SetSelectedVoiceFieldLabel,
  SetUserIdb,
  SetUser,
} from 'src/app/state/auth/auth-state.actions';
...
export class AppService {
  ...
  public isNew;
  public now = new Date().toLocaleString('en-US', {
  ...
  public popVoiceDialog(form, formField, title) {
    ...
    (dialogConfig.maxWidth = '100vw'),
      ...
      this.dialog
        .open(VoiceComponent, dialogConfig)
        .afterClosed()
        .subscribe(() => {
          form.controls[formField].setValue(
            this.store.selectSnapshot(DeviceState.transcription)
          );
        });
  }
  ...
  getCloud() {
    ...
    return this.http.get(
      this.apiUrl +
        tenant.tenant_id +
        '/' +
        this.store.selectSnapshot(AuthState.selectedForm['form_id']) +
        '/'
    );
  }
  ...
  initializeUser() {
    ...
    this.store.select(AuthState.tenant).subscribe((tenant) => {
      this.apiService
        .getLists({ tenant_id: tenant.tenant_id })
        .subscribe((lists) => {
      ...
      this.apiService
        .getEmailList({ tenant_id: tenant.tenant_id })
        .subscribe((lists) => {
      ...
      this.formService.getForms().subscribe((forms: any) => {
        ...
        this.idbCrudService.readAll('form').subscribe((forms: any) => {
          ...
          const formsPublished = forms.filter(
            (form) => form.is_published === true
          );
  ...
  initializeMyNotifications() {
    this.notificationService.getMyNotifications().subscribe((data: any) => {
      ...
      if (data.length == 0)
        this.store.dispatch(new SetNotificationMyCount(undefined));
  ...
  initializeAdminNotifications() {
    this.notificationService.getAllNotifications().subscribe((data) => {
      ...
      this.store.dispatch(
        new SetNotificationAllSigned(this.notificationSigned)
      );
      let count = 0;
